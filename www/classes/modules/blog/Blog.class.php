...
class ModuleBlog extends Module {
	...
	/**
	 * Получает список блогов по количеству юзеров
	 *
	 * @param unknown_type $iLimit
	 * @return unknown
	 */
	public function GetBlogsCountUser($iCurrPage,$iPerPage) {		
		if (false === ($data = $this->Cache_Get("blog_users_count_{$iCurrPage}_{$iPerPage}"))) {				
			$data = array('collection'=>$this->oMapperBlog->GetBlogsCountUser($iCount,$iCurrPage,$iPerPage),'count'=>$iCount);
			$this->Cache_Set($data, "blog_users_count_{$iCurrPage}_{$iPerPage}", array("blog_update","blog_new"), 60*60*24*2);
		}
		$data['collection']=$this->GetBlogsAdditionalData($data['collection'],array('owner'=>array(),'relation_user'));
		return $data;
	}
