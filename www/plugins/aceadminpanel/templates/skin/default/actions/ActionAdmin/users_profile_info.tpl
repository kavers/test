...
function AdminSave(name) {
    ...
    if ((view=$('v_'+name)) && (edit=$('e_'+name))) {
        ...
        if ($('edit_submit')) {$('edit_submit').disabled=true;}
    }
}
    ...
    <table>
        ...
        <tr>
            ...
            <td>
                ...
                <a href="{router page='blog'}{$oBlogUser->getBlogUrl()}/">{$oBlogUser->getBlogTitle()|escape:'html'}</a>{if !$smarty.foreach.blog_user.last}, {/if}
                {/foreach}
                {else}
                {$oLang->adm_no}
                {/if}
