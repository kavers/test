...
<script type="text/javascript" src="{cfg name='path.static.skin'}/js/highslide.packed.js"></script>
    ...
    <li id="vip_blogs" class="block2 green">
        <div class="title"><a href="{router page='blog'}widgets/{$oBlog->getId()}/all/" class="link"><h1>Виджеты</h1></a>{if $aLangCategory}<img src="{cfg name='path.static.skin'}/img/arrow.gif" width="7" height="12" alt="arrow" title="arrow" class="arrow"/><a href="" class="link"><h1>{$aLangCategory}</h1></a>{/if}</div>
           ...
           <ul class="table_list dop">
            {foreach from=$aWidgets item=oWidget}
	         ...
	         <div class="vipblog_info">
                   ...
                   {if !$oWidget->getWidFav('collective',$oBlog->getId())} <a href="{router page='blog'}widgets/{$oBlog->getId()}/?setfav={$oWidget->getWidId()}"><img src="{cfg name='path.static.skin'}/img/comment_ico5.png" width="24" height="23" alt="" title=""/></a>
                    {else}<a href="{router page='blog'}widgets/{$oBlog->getId()}/?delfav={$oWidget->getWidId()}"><img src="{cfg name='path.static.skin'}/img/cut_ico.png" width="24" height="23" alt="" title=""/></a>{/if}
        	 </div>
              </li>
             {/foreach}
           </ul>
