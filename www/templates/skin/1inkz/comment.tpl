            ...
            <dl class="comment_item level{$cmtlevel}">
              <dt><a href="{$oUser->getUserWebPath()}"><img src="{$oUser->getProfileAvatarPath(24)}" width="47" height="44" alt="avatar" title="avatar"/></a></dt>
              <dd>
                 <p>{$oComment->getText()}</p>
                 <p class="author"><a href="{$oUser->getUserWebPath()}" class="author">{$oUser->getLogin()}</a> {date_format date=$oComment->getDate()}</p>
						...
						{if !$oComment->getDelete() or $bOneComment or ($oUserCurrent and $oUserCurrent->isAdministrator())}
							...
							<div class="info">
								...
								<ul>
									...
									{if $oComment->getPid()}
										<li class="goto-comment-parent"><a href="#comment{$oComment->getPid()}" onclick="return lsCmtTree.goToParentComment($(this));" title="{$aLang.comment_goto_parent}">в†‘</a></li>
									{/if}
									<li class="goto-comment-child hidden"><a href="#" onclick="return lsCmtTree.goToChildComment(this);" title="{$aLang.comment_goto_child}">в†“</a></li>
